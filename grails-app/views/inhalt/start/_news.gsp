...
<g:if test="${newsList}">
	<g:each status="i" var="news" in="${newsList}">
		<tr>
			...
			<td valign="top">&nbsp;</td>
			<td height="25" valign="top"><link:abteilungShow code="${news.abteilung?.code}">${news.ueberschrift ? news.abteilung?.name + ' | ' + news.ueberschrift : news.abteilung?.name}</link:abteilungShow><br><span class="copy">${news.inhalt[0..60]}${news.inhalt.size() > 60 ? '[...]' : ''}</span></td>
		</tr>
		<tr valign="middle">
