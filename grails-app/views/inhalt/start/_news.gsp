			...
			<td height="25" valign="top">
				...
				<g:each status="count" var="news" in="${((Map.Entry)newsEntry)?.getValue()}">
					<p style="margin-top: 0px; margin-bottom: 3px;">
						<g:if test="${news?.abteilung?.oberAbteilung == null}">
							<a href="${createLink(controller: 'abteilung', action: 'aktuelles', params: [code: news?.abteilung?.code], mapping: 'abteilungAktuelles',  fragment: 'bericht_' + news?.id)}" style="text-decoration: none;"> > <span class="copy">${news.ueberschrift ? news.ueberschrift : (news.inhalt[0..60] + (news.inhalt.size() > 60 ? '[...]' : ''))}</span></a>
						</g:if>
						<g:else>
