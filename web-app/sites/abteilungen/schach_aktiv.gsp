<html>
<head>
<meta name="layout" content="sv_leingarten"/>
</head>
<table width="614" border="0" cellpadding="0" cellspacing="0">
  <tbody>
    <tr align="left" valign="top">
      <td width="520" height="227">
        <table width="570" border="0" align="left" cellspacing="5">
          <tbody>
            <tr valign="top">
              <td class="copy">
                <p class="headline">
                  Übungsabend
                </p>
                <p>
                  Zu unseren Übungsabenden sind Gäste herzlich willkommen. 
                    Wir legen Wert auf eine lockere Atmosphäre. In den Sommermonaten 
                    kann man uns auch im Biergarten (Paradies) antreffen.
                  <br/>
                </p>
                <p class="copy">
                   
                </p>
              </td>
              <td>
                <p class="copy">
                  <img src="${resource(dir: 'bilder/abtei/schach', file: 'Schach_2794.jpg')}" width="275" height="179"/>
                </p>
              </td>
            </tr>
            <tr valign="top">
              <td class="copy">
                <p class="headline">
                  Vereinsmeisterschaft
                </p>
                <p>
                  Vereinsmeister 2006 bei den Aktiven wurde Roland Dietz. Die 
                    Vereinsmeisterschaft 2007 startet am 05.06.2007.
                </p>
                <p>
                  <br/>
                </p>
              </td>
              <td>
                 
              </td>
            </tr>
            <tr valign="top">
              <td class="copy">
                <p class="headline">
                  Verbandsrunde
                </p>
                <p>
                  <br/>
                  In der Saison 2007/ 2008 nehmen wir mit folgender Aufstellung 
                    an der Verbandsrunde teil:
                </p>
              </td>
              <td>
                 
              </td>
            </tr>
            <tr valign="top">
              <td class="copy">
                <p class="wichtig">
                  1. Mannschaft (C-Klasse)
                </p>
                <table width="275" border="0" cellspacing="0">
                  <tbody>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          1.
                        </div>
                      </td>
                      <td>
                        Armin Holzinger
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          2.
                        </div>
                      </td>
                      <td>
                        Markus Holzinger
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          3.
                        </div>
                      </td>
                      <td>
                        Oliver Schwarz
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          4.
                        </div>
                      </td>
                      <td>
                        Roland Dietz
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          5.
                        </div>
                      </td>
                      <td>
                        Rolf Keller
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          6.
                        </div>
                      </td>
                      <td>
                        Timo Gehring
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          7.
                        </div>
                      </td>
                      <td>
                        Stanislav Cucek
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          8.
                        </div>
                      </td>
                      <td>
                        Hartwig Klamroth
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          9.
                        </div>
                      </td>
                      <td>
                        Heinz Macco
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          10.
                        </div>
                      </td>
                      <td>
                        Marco Hofmann
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          11.
                        </div>
                      </td>
                      <td>
                        Udo Wörsching
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          12.
                        </div>
                      </td>
                      <td>
                        Chris Dörr
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          13.
                        </div>
                      </td>
                      <td>
                        Daniel Martel
                      </td>
                    </tr>
                    <tr class="copy">
                      <td>
                        <div align="right">
                          14.
                        </div>
                      </td>
                      <td>
                        Wolfgang Schwarz
                      </td>
                    </tr>
                    <tr class="copy">
                      <td>
                        <div align="right">
                          15.
                        </div>
                      </td>
                      <td>
                        Hans Schmitt
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          16.
                        </div>
                      </td>
                      <td>
                        Lucas Hermann
                      </td>
                    </tr>
                  </tbody>
                </table>
                <p>
                  <br/>
                </p>
                <p>
                   
                </p>
              </td>
              <td>
                <p>
                  <span class="wichtig">
                    2. Mannschaft (D-Klasse)
                  </span>
                </p>
                <table width="275" border="0" cellspacing="0">
                  <tbody>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          1.
                        </div>
                      </td>
                      <td>
                        Chris Dörr
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          2.
                        </div>
                      </td>
                      <td>
                        Daniel Martel
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          3.
                        </div>
                      </td>
                      <td>
                        Timo Gehring
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          4.
                        </div>
                      </td>
                      <td>
                        Hans Schmitt
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          5.
                        </div>
                      </td>
                      <td>
                        Wolfgang Schwarz
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          6.
                        </div>
                      </td>
                      <td>
                        Hermann, Eric
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          7.
                        </div>
                      </td>
                      <td>
                        Lorenz Martel
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          8.
                        </div>
                      </td>
                      <td>
                        Christian Deme
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          9.
                        </div>
                      </td>
                      <td>
                        Lucas Hermann
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          10.
                        </div>
                      </td>
                      <td>
                        Tobias Hermann
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          11.
                        </div>
                      </td>
                      <td>
                        Sebastian Kezic
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          12.
                        </div>
                      </td>
                      <td>
                        Christian Kezic
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right">
                          13.
                        </div>
                      </td>
                      <td>
                        Marcel Szaruga
                      </td>
                    </tr>
                    <tr class="copy">
                      <td>
                        <div align="right">
                          14.
                        </div>
                      </td>
                      <td>
                        Philipp Schaber
                      </td>
                    </tr>
                    <tr class="copy">
                      <td>
                        <div align="right">
                          15.
                        </div>
                      </td>
                      <td>
                        Marvin May
                      </td>
                    </tr>
                    <tr class="copy">
                      <td width="20">
                        <div align="right"/>
                      </td>
                      <td>
                         
                      </td>
                    </tr>
                  </tbody>
                </table>
                <p>
                  <br/>
                </p>
              </td>
            </tr>
          </tbody>
        </table>
      </td>
      <td width="120">
        <table width="120" border="0" cellpadding="3" bordercolor="#93C9FF">
          <tbody>
            <tr>
              <td width="129" height="20" bgcolor="#93BEEE" class="infohead">
                Training
              </td>
            </tr>
            <tr>
              <td valign="bottom" bgcolor="#D2DFFF" class="copy">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tbody>
                    <tr>
                      <td width="40" class="copy">
                        Di
                      </td>
                      <td class="copy">
                        <div align="right">
                          ab 20.00
                        </div>
                      </td>
                    </tr>
                  </tbody>
                </table>
                <br/>
                <a href="${resource(dir: 'anlagen', file: 'heuchelberg_sportheim.gsp')}">
                  Vereinsraum
                </a>
                <br/>
                Gaststätte Paradies,
                <br/>
                1. Stock
              </td>
            </tr>
          </tbody>
        </table>
        <table width="120" border="0" cellpadding="3" bordercolor="#93C9FF">
          <tbody>
            <tr>
              <td height="20" bgcolor="#93BEEE" class="infohead">
                Mannschaftsführer
              </td>
            </tr>
            <tr>
              <td valign="bottom" bgcolor="#D2DFFF" class="copy">
                <p>
                  <img src="${resource(dir: 'bilder/abtei/schach', file: 'Keller_Rolf.jpg')}" width="110" height="110"/>
                  <br/>
                  <span class="wichtig">
                    Rolf Keller
                  </span>
                  <br/>
                  (Mannschaftsführer
                  <br/>
                  1. Mannschaft)
                  <br/>
                  Tel. 07131 / 40 17 07
                </p>
                <p>
                  <img src="${resource(dir: 'bilder/abtei/schach', file: 'Eric_Hermann.jpg')}" width="110" height="165"/>
                  <br/>
                  <span class="wichtig">
                    Eric Hermann
                  </span>
                  <br/>
                  (Mannschaftsführer
                  <br/>
                  2. Mannschaft)
                </p>
              </td>
            </tr>
          </tbody>
        </table>
      </td>
    </tr>
  </tbody>
</table>

</html>